...
hooks:
  after_create: |
    git clone --depth 1 https://github.com/forbidden-game/panmux.git .
  before_remove: |
...
- `Human Review` -> legacy/manual-hold compatibility state; do not use as the default handoff target.
- `Merging` -> automatic merge state once quality gates pass; execute the `land` skill flow (do not call `gh pr merge` directly).
...
3. Route to the matching flow:
   ...
   - `Human Review` -> treat as compatibility state and continue execution readiness checks; move to `Merging` once merge gates pass.
...
- Do not move to `Merging` for GitHub access/auth until all fallback strategies have been attempted and documented in the workpad.
...
13. Once the issue is in `Merging`, run the `land` skill loop until merge succeeds, then move issue to `Done`.
14. For `Todo` tickets that already had a PR attached at kickoff:
    - Ensure all existing PR feedback was reviewed and resolved, including inline review comments (code changes or explicit, justified pushback response).
    - Ensure branch was pushed with any required updates.
    - Then move to `Merging`.

## Step 3: Auto Merge Handling

1. If the issue is in `Human Review`, treat it as a compatibility/manual-hold state:
   - run a fresh PR feedback/checks sweep;
   - if merge gates pass, move it to `Merging`;
   - if actionable feedback exists, move it to `Rework`.
2. When the issue is in `Merging`, use the `land` skill and run it in a loop until the PR is merged. Do not call `gh pr merge` directly.
...
- `Human Review` is compatibility-only; do not use it as the default success handoff state.
